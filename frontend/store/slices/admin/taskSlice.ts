...
export const fetchTasks = createAsyncThunk<FetchTasksResponse, FetchTasksParams, { rejectValue: string }>(
  'admin/tasks/fetchAll',
  async ({ status, search, date, userId, page, pageSize, sortField, sortOrder }, { rejectWithValue }) => {
    try {
...
export const createTask = createAsyncThunk<Task, Partial<Task>, { rejectValue: string }>(
  ...
  async (newTask, { rejectWithValue }) => {
    try {
      const { data } = await api.post('/admin/tasks', newTask)
      return data.data
    } catch (error: any) {
...
export const updateTask = createAsyncThunk<Task, Task, { rejectValue: string }>(
  ...
  async (task, { rejectWithValue }) => {
    try {
      ...
      const { data } = await api.put(`/admin/tasks/${_id}`, taskData)
      return data.data
    } catch (error: any) {
...
export const deleteMultipleTasks = createAsyncThunk<string[], string[], { rejectValue: string }>(
  'admin/tasks/deleteMultiple',
  async (taskIds, { rejectWithValue }) => {
    try {
      await api.post('/admin/tasks/delete-multiple', { taskIds })
      return taskIds
    } catch (error: any) {
